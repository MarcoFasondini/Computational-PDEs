...
\begin{itemize}
\item[2. ] Show that the DFT $Q_n$ is symmetric ($Q_n = Q_n^{\top}$) but not Hermitian ($Q_n \ensuremath{\ne} Q_n^*$).
...
\]
\begin{itemize}
\item[4. ] Prove that the trigonometric interpolant $p_n(x)$ that interpolates $f$ at $x = x_j = jh$, $j  = 0, \ldots, n-1$ with $h = 2\pi/n$ and $n = 2m + 1$ is unique.
